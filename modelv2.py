...
class Util:
    ...
    @staticmethod
    def timeAnnotation(function: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            start_time = time.time()
            result = function(*args, **kwargs)
            end_time = time.time()
            execution_time = end_time - start_time
            print(f"Function '{function.__name__}' executed in {execution_time:.6f} seconds")
            return result
        return wrapper
